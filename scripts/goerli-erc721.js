...
describe("Flashbot contract", () => {
  ...
  let flashAddress = "0x6f81F07Cb047db816BC4ad8298C81e17a5BAaA5A"
  before(async () => {
    ...
    nftAddress = "0x893aAD361D51b159e77E10f07F5Aa799dC16AD8f";
    ...
    const tx = await nftContract.setApprovalForAll(flashAddress, true);
    await tx.wait();
  })

  describe("ERC721 functions should revert if sent to non whitelisted party", async () => {
    it("Transfer", async () => {
    //reset([1,9]);
      expect(await nftContract.isApprovedForAll(user.address, flashAddress)).to.equal(true);
      expect(await nftContract.ownerOf(1)).to.equal(user.address);
      const tx = await nftContract.transferFrom(user.address, nonWl, 1);
      try {
        await tx.wait();
        // If transaction successful (UNINTENDED), force a throw:
        expect(0 == 1);
      } catch {
        expect(await nftContract.ownerOf(1)).to.equal(vaultAddress);
        expect(await nftContract.ownerOf(2)).to.equal(vaultAddress);
